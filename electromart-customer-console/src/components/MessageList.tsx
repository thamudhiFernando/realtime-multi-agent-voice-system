/**
 * Message List Component with Material-UI
 * Enhanced with visual message-response correlation
 */
'use client';
...
import useMessageCorrelation, { getCorrelationColor, getMessageCorrelationInfo } from '../hooks/useMessageCorrelation';
...
export default function MessageList({
  ...
  // Use the optimized correlation hook
  const correlationMap = useMessageCorrelation(messages, pendingMessages);

  // Get correlation number for a message
  ...
  const isMessagePending = (message: Message, index: number): boolean => {
    if (message.role === 'user') {
      const messageId = message.message_id || index;
      return pendingMessages.has(message.message_id || '') ||
             (getMessageCorrelationInfo(messageId, correlationMap)?.isPending || false);
    }
    return false;
  };
      ...
      <Box sx={{ maxWidth: 900, mx: 'auto' }}>
            ...
            >
              ...
              <Box sx={{ maxWidth: '70%' }}>
                ...
                >
                  ...
                  {message.metadata?.type === 'agent_switch' && (
        ...
        {isTyping && (
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            <Avatar
              sx={{
                bgcolor: getAgentColor(currentAgent || undefined),
                mr: 2,
              }}
            >
