/**
 * Main Chat Interface Component with Material-UI
 */
'use client';

import { Box, AppBar, Toolbar, Typography, Chip, Button, Alert } from '@mui/material';
import {
...
export default function ChatInterface() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
              <CircleIcon
                sx={{
                  fontSize: 12,
                  mr: 1,
                  color: isConnected ? STATUS_COLORS.CONNECTED : isReconnecting ? STATUS_COLORS.RECONNECTING : STATUS_COLORS.DISCONNECTED,
      ...
      {/* Error Banner */}
      {error && (
        <Alert severity="error" onClose={() => {}} sx={{ borderRadius: 0 }}>
          {error}
        </Alert>
      )}
