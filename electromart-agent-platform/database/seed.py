...
from .models import Customer, Product, Order, Promotion, SupportTicket, Conversation
...
def seed_products(db: Session) -> list:
    ...
    products = []
    for product_data in kb["products"]:
        product = Product(
...
def seed_orders(db: Session, customers: list, products: list) -> list:
    ...
    orders_data = [
        {
            ...
            "status": "delivered",
            "tracking_number": "TRK001234XYZ",
            "days_ago": 15,
            "delivery_days": 5
        },
        {
            "order_number": "ORD001235",
            "status": "shipped",
            "tracking_number": "TRK001235XYZ",
            "days_ago": 3,
            "delivery_days": 2
        },
        {
            "order_number": "ORD001236",
            "status": "processing",
        ...
        {
            "order_number": "ORD001237",
            "status": "out_for_delivery",
            "tracking_number": "TRK001237XYZ",
            "days_ago": 5,
            "delivery_days": 0
        },
        {
            "order_number": "ORD001238",
            "status": "delivered",
            "tracking_number": "TRK001238XYZ",
            "days_ago": 30,
            "delivery_days": 7
        },
    ]
    ...
    for i, data in enumerate(orders_data):
        ...
        order = Order(
            ...
            order_date=datetime.utcnow() - timedelta(days=data["days_ago"]),
            delivery_date=datetime.utcnow() - timedelta(days=data["days_ago"]) + timedelta(days=data["delivery_days"]) if data["delivery_days"] is not None else None,
...
def seed_promotions(db: Session) -> list:
    ...
    promotions = []
    for promo_data in kb["active_promotions"]:
        promotion = Promotion(
...
def seed_support_tickets(db: Session, customers: list, products: list) -> list:
    ...
    tickets_data = [
        {
            ...
            "description": "Laptop won't boot up after Windows update",
            "status": "open",
            "priority": "high"
        },
        {
            "ticket_number": "TKT12345679",
            "issue_type": "warranty",
            "description": "Phone screen cracked, checking warranty coverage",
            "status": "in_progress",
            "priority": "medium"
        },
        {
            "ticket_number": "TKT12345680",
            "issue_type": "repair",
            "description": "TV has no picture, backlight issue suspected",
            "status": "resolved",
            "priority": "medium"
        },
        {
            "ticket_number": "TKT12345681",
            "issue_type": "setup",
            "description": "Need help connecting soundbar to TV",
            "status": "resolved",
            "priority": "low"
        },
    ]
    ...
    for i, data in enumerate(tickets_data):
        ...
        ticket = SupportTicket(
            ...
            resolved_at=datetime.utcnow() - timedelta(days=random.randint(0, 5)) if data["status"] == "resolved" else None
        )
        db.add(ticket)
